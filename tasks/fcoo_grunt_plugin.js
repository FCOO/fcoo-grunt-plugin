...
module.exports = function(grunt) {
	...
	avaliableTasks.push('check', 'dev');
	if (options.isApplication){
	  options.haveGhPages = false;
		avaliableTasks.push('build', 'release');
	}
	else {
		options.minimizeBowerComponentsJS = false;
		options.minimizeBowerComponentsCSS = false;
		avaliableTasks.push('prod', 'build', 'push', 'push-cli', 'github', 'github-cli');
	}

	//**************************************************************************************
	//Variable for easy syntax
	var isApplication								= !!options.isApplication,
			isPackage										= !isApplication,

			haveStyleSheet							= !!options.haveStyleSheet,
			haveJavaScript							= !!options.haveJavaScript,
			haveGhPages									= !!options.haveGhPages,

			minimizeBowerComponentsJS		= !!options.minimizeBowerComponentsJS,
			minimizeBowerComponentsCSS	= !!options.minimizeBowerComponentsCSS,

			cleanUp											= !!options.cleanUp,
			bowerCheckExistence					= !!options.bowerCheckExistence,
			bowerDebugging							= !!options.bowerDebugging,
...
*/
    ...
    concat: {
	  ...
	  jshint: {
			options	:	merge(
									{	force: !options.exitOnJSHintError },
	...
	grunt.registerTask('default', function() {
		function tastAvaliable( taskName ) {
			for (var i=0; i<avaliableTasks.length; i++ )
				if (avaliableTasks[i] == taskName)
				  return true;
			return false;
		}
		...
		if (tastAvaliable('build'))
			writelnColor('>grunt build ', 'white', '=> Creates a production version in /dist - same as >grunt prod', 'yellow');
		if (tastAvaliable('push'))
			writelnColor('>grunt push  ', 'white', '=> Create a new Github release incl. new version and tag - same as >grunt github', 'yellow');
		if (tastAvaliable('push-cli'))
			writelnColor('>grunt push-cli {OPTIONS} ', 'white', '=> Create a new Github release incl. new version and tag - same as >grunt github-cli {OPTIONS}', 'yellow');
		writelnYellow('*************************************************************************');
	...
	**************************************************/
	...
	grunt.registerTask('_before_prod',	function(){ _runACmd(options.beforeProdCmd);	});
	grunt.registerTask('_before_dev',		function(){ _runACmd(options.beforeDevCmd);		});
	grunt.registerTask('_after_prod',		function(){ _runACmd(options.afterProdCmd);		});
	grunt.registerTask('_after_dev',		function(){ _runACmd(options.afterDevCmd);		});
